...
async def hotel_agent_node(state: MultiAgentState) -> dict:
    ...
    for kw, kind in keyword_rounds:
        try:
            ...
            if not pois:
                continue
            if kind == "hotel":
                #筛选出酒店的poi，extend将酒店追加到hotel_pois
    ...
    # 3.5 poi_id 确定性回填：紧凑清单刻意不含 id（省 token），导致 LLM 甄选结果可能漏填 poi_id。
    _poi_id_map = {p.get("name"): p.get("id") for p in [*hotel_pois, *restaurant_pois] if p.get("id")}
    for cand in [*result.hotels, *result.restaurants]:
        if not cand.poi_id:
            cand.poi_id = _poi_id_map.get(cand.name, "")
